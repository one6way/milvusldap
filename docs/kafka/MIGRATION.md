...
| Файл | Назначение |
|------|------------|
| [values/values-kafka-internal-overlay.yaml](values/values-kafka-internal-overlay.yaml) | Встроенный Kafka (subchart Bitnami) |
...
> **Корпоративный Kafka уже в registry?** См. [IMAGES_AND_REGISTRY.md](IMAGES_AND_REGISTRY.md) — часто **не нужно** класть tar.gz в пакет (сценарий A/B).

Минимум для **встроенного** Kafka (Bitnami subchart):
...
```bash
cp docs/kafka/values/values-kafka-internal-overlay.yaml values/values-kafka-prod.yaml
...
```bash
cp docs/kafka/values/values-external-kafka-overlay.yaml values/values-kafka-prod.yaml
